  ...
  const handleFileSelect = (selectedFile: File | null) => {
    if (selectedFile) {
      if (selectedFile.type !== 'image/jpeg') {
        setInlineError('Unsupported file type. Please upload a .jpg or .jpeg file.');
        setDropError(true);
        setTimeout(() => {
            setDropError(false);
            setInlineError(null);
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="text-center mb-8">
        <h2 className="text-3xl md:text-4xl font-extrabold text-gray-900 dark:text-white">JPG Compressor</h2>
        <p className="mt-2 text-lg text-gray-600 dark:text-gray-400">
          Reduce the file size of your JPG images.
        </p>
      </div>
             ...
             {inlineError ? (
                ...
                <>
                    <p className="mt-4 text-center text-gray-600 dark:text-gray-400">
                      <span className="font-semibold text-indigo-600 dark:text-indigo-400">Click to upload</span> or drag and drop a JPG file
                    </p>
                    <p className="text-sm text-gray-500 dark:text-gray-500">Supported: .jpg, .jpeg</p>
                </>
            )}
            <input type="file" ref={fileInputRef} onChange={(e) => handleFileSelect(e.target.files ? e.target.files[0] : null)} className="hidden" accept="image/jpeg" />
          </div>
        )}
