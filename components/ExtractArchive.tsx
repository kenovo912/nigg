  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="text-center mb-8">
        <h2 className="text-3xl md:text-4xl font-extrabold text-gray-900 dark:text-white">Extract Archive</h2>
        <p className="mt-2 text-lg text-gray-600 dark:text-gray-400">
          Unpack files from a ZIP or other archive format.
        </p>
      </div>
             ...
             {inlineError ? (
                ...
                <>
                    <p className="mt-4 text-center text-gray-600 dark:text-gray-400">
                      <span className="font-semibold text-indigo-600 dark:text-indigo-400">Click to upload</span> or drag and drop an archive
                    </p>
                    <p className="text-sm text-gray-500 dark:text-gray-500">Supported: .zip</p>
                </>
            )}
            <input type="file" ref={fileInputRef} onChange={(e) => handleFileSelect(e.target.files ? e.target.files[0] : null)} className="hidden" accept=".zip,application/zip,application/x-zip-compressed" />
          </div>
        )}
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg min-h-[100px]">
            {isLoading && (
              <div className="flex flex-col items-center">
                <p className="text-center text-gray-600 dark:text-gray-400 mb-2">Extracting... {Math.round(progress)}%</p>
            ...
            {downloadUrl && (
              <div className="text-center">
                ...
                <a href={downloadUrl} download="extracted-files-list.txt" className="inline-flex items-center gap-2 px-6 py-3 bg-green-600 text-white font-semibold rounded-lg shadow-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-all duration-300">
                  <DownloadIcon />
                  Download File List
                </a>
              </div>
